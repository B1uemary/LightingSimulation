...
public static class AmplifyOcclusionCommon
{
	...
	public static void UpdateGlobalShaderConstants( CommandBuffer cb, ref TargetDesc aTarget, Camera aCamera, bool isDownsample )
	{
	#if UNITY_2017_2_OR_NEWER && !UNITY_SWITCH && !UNITY_XBOXONE && !UNITY_PS4
		...
		if( aCamera.orthographic )
			projScale = ( (float)aTarget.height ) / aCamera.orthographicSize;
		else
			projScale = ( (float)aTarget.height ) / ( Mathf.Tan( fovRad * 0.5f ) * 2.0f );
